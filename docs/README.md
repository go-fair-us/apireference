...
This documentation outlines a streamlined way for organizations to share digital objects (like datasets) so they can be easily indexed and discovered. By using a "read-only" web approach, providers serve data in a standardized format called JSON-LD, which acts like a bridge between human-readable text and machine-readable data. Whether you are building a new API from scratch or aligning an existing system, the goal is to use common web standards—such as Sitemaps for discovery and Schema.org for description—to ensure your data can be easily integrated into larger ecosystems like the NIAID Data Explorer or Google Dataset Search.
